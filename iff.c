...
#ifdef DEBUG_LBM
static int depth = 0;
#define PRINT_DEPTH() for(int d = depth; d>0; d--) { printf("  "); }
#endif

#ifdef DEBUG_LBM
const char *chunk_name[N_CHUNKS] = {
    "FORM",
    "BMHD",
    "CMAP",
    "CRNG",
...
void free_BODY(struct ck_BODY *c) {
    free(c->body);
}
...
struct chunk *parseCMAP(const void *data, size_t size) {
    ...
    for(int i = 0; i < print_length; i++) {
        PRINT_DEPTH() printf("  { r: %02x, g: %02x, b: %02x }\n", ck->ColorMap[i].r, ck->ColorMap[i].g, ck->ColorMap[i].b);
    }
